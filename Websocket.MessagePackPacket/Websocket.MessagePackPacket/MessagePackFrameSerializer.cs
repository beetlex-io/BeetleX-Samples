...
namespace Websocket.MessagePackPacket
{
    ...
    {
        public static HttpApiServer RegisterMessagePack<T>(this HttpApiServer server)
        {
            MessagePackFrameSerializer.BinaryDataFactory.RegisterComponent<T>();
            return server;
        }
        public static HttpApiServer UseMessagePackController(this HttpApiServer server, Action<WebSocketReceiveArgs> handler = null)
        {
            server.WebSocketReceive = async (o, e) =>
            {
                try
                {
                    ...
                    var action = MessagePackFrameSerializer.BinaryDataFactory.GetHandler(msg);
                    if (action != null)
                    {
                        if (!action.IsVoid)
                        {
    ...
    public class MessagePackFrameSerializer : IDataFrameSerializer
    {
